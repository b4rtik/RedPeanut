...
namespace RedPeanut
{
    public class PersStartupManager : IMenu
    {
        ...
        string fileName = "startup.bat";
        bool encoded = false;
        bool ssl = true;
        bool exit = false;

        public PersStartupManager()
        {

        }

        public PersStartupManager(IAgentInstance agent)
        {
            this.agent = agent;
        }
        ...
        private void Run()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(fileName))
                {
                    ...
                    string perssrc = File.ReadAllText(Path.Combine(folderrpath, PERSSTARTUP_TEMPLATE));

                    perssrc = Replacer.ReplacePersStartup(perssrc, url, fileName, encoded);
